        ...
        <div className="card dashboard-visual-card">
          <div className="card-header">
            <div>
              ...
              <div className="card-subtitle">Mô hình trực quan theo hệ thống BESS: Grid, PV, PCS, Battery và Load.</div>
            </div>
          </div>

          <div className="visual-kpi-row">
            <div className="visual-kpi-box">
              <span className="visual-kpi-icon"><LuBatteryCharging /></span>
              <div>
            ...
            <div className="visual-kpi-box">
              <span className="visual-kpi-icon"><LuShield /></span>
              <div>
            ...
            <div className="visual-kpi-box">
              <span className="visual-kpi-icon"><LuBadgeAlert /></span>
              <div>
          ...
          <div className="visual-scene">
            <div className="visual-scene-canvas">
              ...
              <div className="visual-info visual-info-left">
                Grid: <strong>{sys.gridVoltage} kV / {sys.gridFrequency} Hz</strong>
              </div>

              <div className="visual-stack visual-info-center">
                <div className="visual-info">Trạng thái hệ thống: <strong>{sys.systemStatus}</strong></div>
                <div className="visual-info">PCS AC Power: <strong>{sys.pcsACPower} kW</strong></div>
                <div className="visual-info">DC Bus: <strong>{sys.batteryVoltage} V</strong></div>
              </div>
              ...
              <div className="visual-stack visual-info-bottom-center">
                <div className="visual-info">Battery Power: <strong>{sys.batteryPower} kW</strong></div>
                <div className="visual-info">SOC / SOH: <strong>{sys.soc}% / {sys.soh}%</strong></div>
              </div>
      ...
      <section className="dashboard-section mt-base">
        <div className="section-heading">
          <div>
            ...
            <p className="card-subtitle">Các chỉ số chính cần theo dõi nhanh.</p>
          </div>
        </div>

        <div className="kpi-grid kpi-grid-compact">
          <KPICard icon={<LuActivity />} title="System Status" value={sys.systemStatus} status={sys.systemStatus} />
          <KPICard icon={<LuGauge />} title="SOC" value={`${sys.soc}%`} progress={sys.soc} progressColor="green" clickable onClick={() => navigate('/battery')} />
          <KPICard icon={<LuBatteryCharging />} title="Battery Power" value={sys.batteryPower} unit="kW" status={sys.batteryPower < 0 ? 'Charging' : 'Discharging'} clickable onClick={() => navigate('/battery')} />
          <KPICard icon={<LuSun />} title="PV Power" value={sys.pvPower} unit="kW" />
          <KPICard icon={<LuCable />} title="Grid Power" value={sys.gridPower} unit="kW" status={sys.gridStatus} />
          <KPICard icon={<LuShield />} title="PCS Status" value={sys.pcsStatus} status={sys.pcsStatus} />
        </div>
      </section>
        ...
        <div className="card">
          <div className="card-header">
            <div>
              ...
              <div className="card-subtitle">Xu hướng công suất của Battery, Grid, PV và Load.</div>
            </div>
          </div>
          <ResponsiveContainer width="100%" height={280}>
            <LineChart data={mockHourlyData}>
              ...
              <Tooltip />
              <Legend />
              <Line type="monotone" dataKey="batteryPower" name="Battery" stroke="#0EA5E9" strokeWidth={2} dot={false} />
              <Line type="monotone" dataKey="gridPower" name="Grid" stroke="#1677FF" strokeWidth={2} dot={false} />
              <Line type="monotone" dataKey="pvPower" name="PV" stroke="#F59E0B" strokeWidth={2} dot={false} />
              <Line type="monotone" dataKey="loadPower" name="Load" stroke="#22C55E" strokeWidth={2} dot={false} />
            </LineChart>
          </ResponsiveContainer>
        </div>
        ...
        <div className="card">
          <div className="card-header">
            <div>
              ...
              <div className="card-subtitle">Các cảnh báo đang hoạt động gần nhất.</div>
            </div>
            <button className="btn btn-ghost btn-sm" onClick={() => navigate('/alarms')}>
              Xem tất cả
            </button>
          </div>
