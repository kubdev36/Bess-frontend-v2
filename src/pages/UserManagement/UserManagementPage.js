...
export default function UserManagementPage() {
  ...
  const saveUser = () => {
    ...
    if (editing) {
      setUsers(prev => prev.map(user => (user.id === editing.id ? { ...user, name: form.name, email: form.email, role: form.role, status: form.status } : user)));
    } else {
      setUsers(prev => [
        {
          id: prev.length + 1,
      ...
      <div className="card">
        <div className="page-toolbar">
          <div>
            ...
            <div className="card-subtitle">Quản lý tài khoản, role, trạng thái và hành động reset password.</div>
          </div>
          <div className="page-toolbar-actions">
            ...
            <select className="form-select" style={{ width: 140 }} value={role} onChange={e => setRole(e.target.value)}>
                  ...
                  <td>
                    <div className="table-actions">
                      ...
                      <button
                        ...
                        onClick={() => setUsers(prev => prev.map(item => (item.id === user.id ? { ...item, status: item.status === 'Locked' ? 'Active' : 'Locked' } : item)))}
                      >
